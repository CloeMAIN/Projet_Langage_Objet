    ...
    while (window->pollEvent(event))
    {
        ...
        else if (event.type == sf::Event::TextEntered)
        {
            ...
            if(event.mouseButton.button == sf::Mouse::Left){
                ...
                if (event.mouseButton.x >= TAILLE_BOUTON_QUITTER_QUESTIONNAIRE.GaucheHaut.x && event.mouseButton.x <= TAILLE_BOUTON_QUITTER_QUESTIONNAIRE.DroiteBas.x && event.mouseButton.y >= TAILLE_BOUTON_QUITTER_QUESTIONNAIRE.GaucheHaut.y && event.mouseButton.y <= TAILLE_BOUTON_QUITTER_QUESTIONNAIRE.DroiteBas.y){
                    *etape = 4;
                    return;
                }
            }
